...
import {MouseMove} from './componentes/MouseMove.jsx'
...
function App() {
  ...
  return (
    ...
    <main>
      ...
      <section className='tablero-lista'>
...

        {
          tablero.map((casilla, index) => {
            return <Tablero key={index} index={index} rellenar={rellenar}><p>{casilla}</p></Tablero>
          })
        }
        </article>
      ...
      <section className='tablero-lista'>
        <Tablero isTurno={turn===TURNS.x}><p>{TURNS.x}</p></Tablero>
        <Tablero isTurno={turn===TURNS.o}><p>{TURNS.o}</p></Tablero>
      </section>
